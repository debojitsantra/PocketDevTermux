...
show_profiles() {
  ...
  echo ""
  printf "  What kind of programmer do you want to be?\n"
  printf "  ${DIM}Pick one or more numbers separated by spaces. Example: 1 3 7${R}\n\n"
...
setup_editor() {
  section "Code Editor"
  echo ""
  printf "  You'll need a text editor to write code in the terminal.\n"
  printf "  ${DIM}Not sure? Pick micro -- it works just like a normal editor.${R}\n\n"
  printf "  ${CYAN}[1]${R}  micro   ${DIM}-- beginner-friendly (Ctrl+S save, Ctrl+Q quit)${R}\n"
...
setup_extras() {
  ...
  echo ""
  printf "  A few extra tools that make your life easier.\n"
  printf "  ${DIM}All optional -- skip anything you don't need.${R}\n\n"
...
setup_vscode_server() {
  ...
  echo ""
  printf "  ${WHITE}code-server${R} runs VS Code in your browser -- full IDE experience\n"
  printf "  on your phone. Open it at ${CYAN}http://localhost:8080${R} after install.\n\n"
  printf "  ${DIM}Needs ~300MB disk space and Node.js.${R}\n\n"
...
VSLAUNCH
  ...
  info "Then open http://localhost:8080 in any browser on your phone."
}
...
setup_ai_assistant() {
  ...
  echo ""
  printf "  Get AI help right in your terminal -- explain code, fix bugs,\n"
  printf "  generate functions, and more. All free options.\n\n"
  printf "  ${DIM}Pick one (or skip -- you can install these manually later):${R}\n\n"
  printf "  ${CYAN}[1]${R}  aichat       ${DIM}-- chat with GPT/Claude/Gemini/Ollama from terminal${R}\n"
...
setup_local_llm() {
  ...
  echo ""
  printf "  Run an AI coding model ${WHITE}completely offline${R} on your device.\n"
  printf "  No internet, no API key, no cost -- runs locally via Ollama.\n\n"
  printf "  ${YELLOW}(i)${R}  ${DIM}Needs a decent phone with 4GB+ free RAM.${R}\n"
  printf "  ${DIM}    Recommended models: qwen2.5-coder:1.5b (small, fast)${R}\n"
  printf "  ${DIM}                        codellama:7b (more capable, slower)${R}\n\n"
...
setup_linux_container() {
  ...
  echo ""
  printf "  Run a ${WHITE}full Linux distro inside Termux${R} using proot-distro.\n"
  printf "  You get apt/dnf, systemd-like services, and a proper Linux environment.\n\n"
  printf "  ${DIM}No root needed. Uses proot -- not a VM, but close enough.${R}\n\n"
...
setup_project_templates() {
  ...
  echo ""
  printf "  Instantly scaffold a new project with the right folder structure,\n"
  printf "  config files, and a working example -- ready to code in seconds.\n\n"
  printf "  ${DIM}This installs a 'newproject' command you can use anytime.${R}\n\n"
...
scaffold_rust() {
  ...
  if command -v cargo &>/dev/null; then
    ...
    printf "  cargo not found. Install Rust first: bash devsetup.sh\n" >&2
    return 1
  fi
}
...
show_resources() {
  ...
  printf "  ${DIM}These run outside Termux -- great for editing files visually${R}\n\n"
  ...
  printf "  ${CYAN}${BOLD}Mimo${R}  ${DIM}-- bite-sized lessons, beginner-friendly, daily streaks${R}\n"
  ...
  printf "  ${CYAN}${BOLD}Grasshopper${R}  ${DIM}-- Google's free JS learning app, great for absolute beginners${R}\n"
  ...
  printf "  ${DIM}Open these in your browser anytime${R}\n\n"
  ...
  printf "  ${CYAN}${BOLD}Khan Academy (Computing)${R}  ${DIM}-- great for absolute beginners, very visual${R}\n"
  ...
  printf "  ${DIM}Build real skills by solving problems${R}\n\n"
...
print_summary() {
  ...
  printf "       ${YELLOW}bash devsetup.sh${R}\n\n"

  hr '-' "$DIM"
  printf "\n  ${MAGENTA}${BOLD}Tip:${R}  The best way to learn is to build something.\n"
  printf "  Pick a small project idea and start today -- even a\n"
  printf "  simple calculator or to-do list teaches you a lot.\n\n"
...
main() {
  ...
  printf "  ${DIM}* Safe to re-run anytime to add more languages.${R}\n"
  printf "  ${DIM}* A full log is saved to ~/devsetup.log${R}\n"
  echo ""
  press_enter

  # Update
  section "Updating Termux packages"
  printf "  ${DIM}Grabbing the latest package lists -- hang tight...${R}\n\n"
  step "pkg update"
  ...
  echo ""
  printf "  The next few steps are ${WHITE}completely optional${R} power features.\n"
  printf "  Skip anything you don't need -- you can always re-run later.\n"
  echo ""
  press_enter
